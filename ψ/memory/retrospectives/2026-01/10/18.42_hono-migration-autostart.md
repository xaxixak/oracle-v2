...
The iterative fixes (stale PID cleanup → lock file → orphan lock detection) show how production-ready code requires multiple layers of defensive programming. Each fix addressed a real failure mode.

## Lessons Learned

- **Pattern**: Auto-start daemons need multiple layers of protection: health check → PID file → lock file → orphan detection
